...
// const initSK = require('./enterprisecopilot.js');
const {RunInSemanticKernel,RunPlanner} = require('./enterprisecopilot.js');
// const RunPlanner = require('./enterprisecopilot.js');
...
function activate(context) {
	...
	let checkcode_cmd = vscode.commands.registerCommand('copilotext.checkcode', async function () {
		...
		this.webView.webview.postMessage({ type: 'addCode', value: '🤖: <br/>' + result + '<br/>'});

		
	});

	let introcode_cmd = vscode.commands.registerCommand('copilotext.introcode', async function () {
		
		const selectedText = vscode.window.activeTextEditor.document.getText(vscode.window.activeTextEditor.selection);

		const result = await RunInSemanticKernel(selectedText,"Intro");

		this.webView.webview.postMessage({ type: 'addCode', value: '🤖: <br/>' + result + '<br/>'});
	});

	let regencode_cmd = vscode.commands.registerCommand('copilotext.regencode', async function () {
		
		const selectedText = vscode.window.activeTextEditor.document.getText(vscode.window.activeTextEditor.selection);

		const result = await RunInSemanticKernel(selectedText,"Regencode");

		this.webView.webview.postMessage({ type: 'addCode', value: '🤖: <br/>' + result + '<br/>'});
	});

	context.subscriptions.push(checkcode_cmd);
	context.subscriptions.push(introcode_cmd);
	context.subscriptions.push(regencode_cmd);
